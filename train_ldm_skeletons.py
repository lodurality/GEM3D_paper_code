#adpated from https://github.com/1zb/3DShape2VecSet
...
from engine_ldm import train_one_epoch_skel, evaluate_skel

def get_args_parser():
    parser = argparse.ArgumentParser('Skeleton diffusion Diffusion', add_help=False)
...
def main(args):
    ...
    if args.eval:
        test_stats = evaluate_skel(data_loader_val, model, device, args=args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_skel(
        ...
        if epoch % 5 == 0 or epoch + 1 == args.epochs:
            test_stats = evaluate_skel(data_loader_val, model, criterion, device, args=args)
            print(f"loss of the network on the {len(dataset_val)} test images: {test_stats['loss']:.3f}")
